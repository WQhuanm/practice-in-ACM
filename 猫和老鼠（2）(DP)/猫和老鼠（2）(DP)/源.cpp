...
int j[1100]; //豆子
int f[1100];  //猫粮
int dp[1100];  //dp[i][k]表示第i间房间剩余k猫粮时的共换取最大豆子的最优解
...
int main() {
	...
	while (scanf("%d %d", &M, &N) == 2 && (N != -1 || M != -1)) {
		...
		for (int i = 1; i <= N; ++i) {
			for (int k =M; k >=f[i]; --k) {  //从M开始，避免后效性
				
					dp[k] = max(dp[k], dp[k - f[i]] + j[i]);  //区别填满，填满是前面有填过（即不为初始化的-1，即每次都是装满的）
			}                           //而我们这个不加条件，全部都加，选最优，就不属于填满的
		}
		printf("%d\n", dp[M]);
